...
export class BookingService {
  ...
  static async getAll() {
    const bookings = await BookingModel.find().populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return bookings
  }

  static async get(id : string) {
    const booking = await BookingModel.findById(id).populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return booking
  }
  ...
  static async getByArtist(artist : string) {
    const booking = await BookingModel.find({artist}).populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return booking
  }

  static async getByBussiness(bussiness : string) {
    const booking = await BookingModel.find({bussiness}).populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return booking
  }

  static async getByBussinessCompleted(bussiness : string) {
    const booking = await BookingModel.find({bussiness, status : "completed"}).populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return booking
  }

  static async getByClient(client : string) {
    const booking =  await BookingModel.find({ client }).populate('artist').populate('client').populate('bussiness').sort({ _id: -1 })  ;
    return booking
  }
  ...
  static async deductBalance(id : string, amount : number) {
    const booking = await BookingModel.findByIdAndUpdate(id,  { $inc: { balance: -amount } });
    return booking
  }
  ...
  static async bookNextSession(id : string , newTime :  string[], newDate : string) {
   ...
   static async bookNextSessionV2(id : string , newTime :  string[], newDate : string) {
    const booking = await BookingModel.findById(id);
    if(!booking) return
  ...
  static async appointmentToSession(data : {
   ...
   return await BookingModel.findByIdAndUpdate(data.id, {
