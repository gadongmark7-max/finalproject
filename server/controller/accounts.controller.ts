...
import { getTime, getDate, getThisMonthSales, getYearlySales, sendEmail } from "../utils/customFunction";
...
export class AccountController {
    ...
    static getBussinessInfo = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
    ...
    static forgotPasswordOtp = async (request : AuthRequest , response : Response) => {
        ...
        sendEmail(account.email!, "Forgot Password OTP", `Your reset code is ${pin}. This code is valid for a limited time.`)
    ...
    static updateForgotPassword = async (request : AuthRequest , response : Response) => {
    ...
    static getBussinessDashboard = async (request : AuthRequest , response : Response) => {
        ...
        const pendingBookings = await BookingService.getByStatusAndStatusLenght(id, "pending")
        const activeBookings = await BookingService.getByStatusAndStatusLenght(id,"active" )
        const activeAppointments = await BookingService.getByStatusAndStatusLenght(id, "appointment")
        ...
        response.send({
            ...
            totalTax: getTax(),
            employee: bussinessInfo?.employees.length! + bussinessInfo?.artists.length!,
    ...
    static EditEmployeeRole = async (request : AuthRequest , response : Response) => {
    ...
    static getClientsAccount = async (request : AuthRequest , response : Response) => {
        const accounts = await AccountService.getByAccountType("client")
        response.send(accounts)
    }

    static getArtistApplicationByBussiness = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const applications = await ArtistApplicationService.getByBussiness(id)
        response.send(applications)
    }

    static getArtistBussiness = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
    ...
    static getAllArtistInfo = async (request : AuthRequest , response : Response) => {
        const artistInfo = await ArtistInfoService.getAll()
        response.send(artistInfo)
    }

    static getAllBussinessInfo = async (request : AuthRequest , response : Response) => {
    ...
    static createBusinessLeave = async (request : AuthRequest , response : Response) => {
    ...
    static getBusinessLeave = async (request : AuthRequest , response : Response) => {
    ...
    static jobPost = async (request : AuthRequest , response : Response) => {
        ...
        await BussinessInfoService.updateJobPost(id, isLookingArtist, jobDescription)
        response.send("sucess")
    }
    ...
    static updateArtistLocation = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const { location, artistId } = request.body
        const account = await AccountService.updateLocation(id, location)

        const accountInfo = (account?.type == "artist") ? await ArtistInfoService.get(artistId) : await BussinessInfoService.get(artistId)
    ...
    static getBussinessFinance = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const expencess = await ExpencesService.getByAccount(id)
        const transactions = await TransactionService.getByReceiver(id)
        const bussinessInfo = await BussinessInfoService.getByBussiness(id)
    ...
    static getAccountAttendance = async (request : AuthRequest , response : Response) => {
        const { email } = request.params
        const attendances = await AttendanceService.getByEmail(email)
        response.send(attendances)
    }

    static getBussinessAttendance = async (request : AuthRequest , response : Response) => {
        const account = request.account
        const attendances = await AttendanceService.getByBussiness(account?._id!)
        response.send(attendances)
    }

    static timeInAttendance = async (request : AuthRequest , response : Response) => {
    ...
    static timeOutAttendance = async (request : AuthRequest , response : Response) => {
        const { timeOut, attendanceId, bussinessId, employeeEmail} = request.body
        const account = request.account
        let employeeShipDuration = 0
        if(account?.type == "artist"){
            const bussinessInfo = await BussinessInfoService.getByBussiness(bussinessId)
            bussinessInfo?.artists.forEach((item) => {
                if(item.artist._id.toString() == account._id)  employeeShipDuration = (item?.schedTime.length!) - 1
            })
        } else {
        ...
        await AttendanceService.updateTimeOut(attendanceId, timeOut, employeeShipDuration)
        response.send("success")
    }
    ...
    static getSenderTransactions = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const transactions = await TransactionService.getBySender(id)
        response.send(transactions)
    }

    static getReceiverTransactions = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const transactions = await TransactionService.getByReceiver(id)
        response.send(transactions)
    }

    static getAccountNotifications = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
    ...
    static getAccountNotificationUnseen = async (request : AuthRequest , response : Response) => {
    ...
    static getAdminMessageUnseen = async (request : AuthRequest , response : Response) => {
    ...
    static artistApplicationApproval = async (request : AuthRequest , response : Response) => {
        ...
        const bussinessInfo = await BussinessInfoService.getByBussiness(account?._id!)
        response.send(bussinessInfo)
    }
     ...
     static addEmployee = async (request : AuthRequest , response : Response) => {
        ...
        await BussinessInfoService.pushEmployee(account?._id!, employee._id.toString(), employeeInfo._id.toString(), accountData)
     ...
     static updateEmployeeInfo = async (request : AuthRequest , response : Response) => {
        const {type, info, employeeId, businessId} = request.body
        switch(type){
            case "employee": 
                await BussinessInfoService.updateEmployeeInfo(businessId, employeeId, info )
            break;

            case "artist": 
                await BussinessInfoService.updateArtistInfo(businessId, employeeId, info )
                console.log("art")
            break;
        }
        response.send("success")
     ...
     static removeEmployee = async (request : AuthRequest , response : Response) => {
        ...
        switch(type){
            case "employee": 
                await BussinessInfoService.removeEmployeeFromBusiness(businessId, employeeId)
                await AccountService.updateAccountType(employeeId, "client")
                await EmployeeInfoService.delete(employeeId, businessId)
            break;

            case "artist": 
                await BussinessInfoService.removeArtistFromBusiness(businessId, employeeId )
            break;
        }
        response.send("success")
    }
    ...
    static createPayroll = async (request : AuthRequest , response : Response) => {
        ...
        if(isApprove){
            const aprovedPayroll = await PayrollService.approve(res._id.toString()!, payroll.preparedBy)
    ...
    static submitPayslipProof = async (request : AuthRequest , response : Response) => {
            try {
                if (!request.file) {
    ...
    static getBussinessPayroll = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
    ...
    static getPayslip = async (request : AuthRequest , response : Response) => {
        ...
        const accountPayroll = bussinessPayrols.flatMap(payroll => 
            payroll.payroll.filter(item => item.employeeId.toString() === account?._id.toString())
        );
     ...
     static editSchedule = async (request : AuthRequest , response : Response) => {
        ...
        switch(type){
            ...
            case  "bussiness_artist":
                await BussinessInfoService.updateArtistSched(account?._id!, id, day, time, salary, salaryType, commision)
            break;

            case  "bussiness_employee":
                await EmployeeInfoService.updateSched(id, day, time, salary, salaryType)
            break
        }
        response.send("success")
    }
    ...
    static addRole = async (request : AuthRequest , response : Response) => {
        ...
        const account = request.account
        await BussinessInfoService.pushRolesToBussiness(account?._id!, role, permissions)
        response.send("success")
    }
    
    

    static artistApplyToBussiness = async (request : AuthRequest , response : Response) => {
        const { id } = request.params
        const account = request.account
    ...
    static getAllArtistVerification= async (request : AuthRequest , response : Response) => {
        const artistVerifications = await ArtistVerificationServices.getAll()
        response.send(artistVerifications)
    }

    static artistVerification = async (request : AuthRequest , response : Response) => {
    ...
    static bussinessVerification = async (request : AuthRequest , response : Response) => {
        const { verificationId, accountId, status, reason } = request.body
        ...
        if(status == "approve"){
            ...
            await AccountService.updateAccountName(accountId, verification?.bussinessName!)
    ...
    static uploadProfileInfoImg = async (request : AuthRequest , response : Response) => {
            try {
                if (!request.file) {
                    response.status(400).json({ error: 'No file uploaded' });
                    return;
    ...
    static changePofilePicture = async (request : AuthRequest , response : Response) => {
            try {
                if (!request.file) {
                    response.status(400).json({ error: 'No file uploaded' });
                    return;
                ...
                const updatedAccount = await AccountService.updateProfilePic(account?._id!, url)
    ...
    static placeReview = async (request : AuthRequest , response : Response) => {
            try {
                ...
                const { bookingId, rating, comment, artistId, bussinessId } = request.body;
                ...
                if(bussinessId != "none"){
                    await BussinessInfoService.pushReviewToBussiness(bussinessId, url, comment, Number(rating), account?._id!)
                    await NotificationService.create({
                        account : bussinessId,
                        date : getDate(),
                        time : getTime(),
                ...
                await ArtistInfoService.pushReviewToArtist(artistId, url, comment, Number(rating), account?._id!)
     ...
     static submitArtistVerification = async (request : AuthRequest , response : Response) => {
            try {
                if (!request.file) {
                    response.status(400).json({ error: 'No file uploaded' });
                    return;
    ...
    static  subsPayment = async (request : AuthRequest , response : Response) => {
            try{
                ...
                if(!transaction){
                    ...
                    const updatedAccount = await AccountService.addSubcriptionDays(sender, days)
    ...
    static submitBussinessVerification = async (request: AuthRequest, response: Response) => {
        try {
            ...
            const BarangayClearanceUpload = await cloudinary.uploader.upload(files.BarangayClearance[0].path, { folder: 'nextjs_uploads', resource_type: 'auto' })
            const businessPermitUpload = await cloudinary.uploader.upload(files.businessPermit[0].path, { folder: 'nextjs_uploads', resource_type: 'auto' })

             const  {bussinessName, expirationDate, clearanceExpiration } = request.body
    ...
    static submitAdminMessage = async (request : AuthRequest , response : Response) => {
            try {
                if (!request.file) {
                    response.status(400).json({ error: 'No file uploaded' });
                    return;
