...
export default function OtpPage() {
  ...
  const resendMutation = useMutation({
    mutationFn: () => axiosInstance.post("/auth/resend", { id: paramsId }),
    onSuccess: () => {
      setTimer(30);
    },
    onError: (err: { request: { response: string } }) => errorAlert("error"),
  });
            ...
            <div className="flex items-start gap-3 bg-danger-muted border border-danger-border px-4 py-3">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <p className="text-danger-light text-sm leading-relaxed opacity-80">
                  The code you entered is incorrect or has expired. Please try
                  again or request a new code below.
                </p>
              </div>
            </div>
          )}
