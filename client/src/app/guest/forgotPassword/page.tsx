...
export default function ForgotPasswordPage() {
  ...
  const resendMutation = useMutation({
    mutationFn: () =>
      axiosInstance.post("/auth/forgotPassword/otp", { email }),
    onSuccess: () => { successAlert("New code sent"); setTimer(30); },
    onError: (err: { request: { response: string } }) =>
      errorAlert(err.request.response || "error"),
  });
  ...
  const iconBase = "absolute left-3.5 w-3.5 h-3.5 text-border pointer-events-none transition-colors duration-200";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-primary px-6 py-14 relative overflow-hidden">
      ...
      <div className="flex items-center gap-2 absolute top-2 left-2">
        <Link href={"/"} className="flex h-24 w-42 items-center justify-center rounded-lg hover:scale-95">
          ...
          <div className="flex items-center justify-center gap-3">
            {steps.map((s, i) => (
              <div key={s.n} className="flex items-center gap-3">
                {i > 0 && <div className={`w-8 h-px ${step >= s.n ? "bg-gold" : "bg-border"}`} />}
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-[10px] uppercase tracking-[0.18em] ${step >= s.n ? "text-gold" : "text-text-muted"}`}>
                    {s.label}
                  </span>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-text-muted text-sm leading-relaxed max-w-xs">
                {step === 1 && "Enter the email linked to your account and we will send you a reset code."}
                {step === 2 && `A one-time passcode has been sent to ${email}.`}
                {step === 3 && "Your code is verified. Choose a new password for your account."}
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              {isCorrect === false && (
                <div className="flex items-start gap-3 bg-danger-muted border border-danger-border px-4 py-3">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-danger-light">Invalid Code</span>
              ...
              {isCorrect === true && (
                <div className="flex items-start gap-3 bg-success-muted border border-success-border px-4 py-3">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-success-light">Identity Verified</span>
              ...
              {isCorrect !== true && (
                ...
                >
                  <RotateCcw size={12} />
                  {timer > 0 ? <span>Resend available in {timer}s</span> : <span>Resend Code</span>}
                </button>
              )}
            </div>
          )}
