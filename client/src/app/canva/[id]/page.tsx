...
const TattooEditor: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { design: designInterface; screenShot: string }) =>
      axiosInstance.post("/works", data),
    onSuccess: () => {
  ...
  const saveCanva = async () => {
    ...
    router.push("/pages/artist/works");
  };
      ...
      img.onload = () => {
        setLayers((prev) => {
          const newLayers = prev.map((layer) =>
            layer.id === prevEntry.layerId ? { ...layer, image: img } : layer,
          );

          // ✅ Force re-render by updating the image reference
          const stage = stageRef.current;
          if (stage) {
            const imageNode = stage.findOne(
              `#${prevEntry.layerId}`,
            ) as Konva.Image;
            if (imageNode) {
              imageNode.image(img);
  ...
  const redo = () => {
    ...
    if (nextEntry.type === "layers") {
      ...
      img.onload = () => {
        setLayers((prev) => {
          const newLayers = prev.map((layer) =>
            layer.id === nextEntry.layerId ? { ...layer, image: img } : layer,
          );
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      ...
      if (!check.success) {
        errorAlert(
          check.error.issues[0]?.message ?? "That image can't be used",
        );
        e.target.value = "";
        return;
      }
  ...
  const flipHorizontal = () => {
    if (!selectedId) return;
    const newLayers = layers.map((layer) =>
      layer.id === selectedId ? { ...layer, scaleX: -layer.scaleX } : layer,
    );
    updateLayersHistory(newLayers);
  };
  ...
  const flipVertical = () => {
    if (!selectedId) return;
    const newLayers = layers.map((layer) =>
      layer.id === selectedId ? { ...layer, scaleY: -layer.scaleY } : layer,
    );
    updateLayersHistory(newLayers);
  };
        ...
        switch (brushType) {
          ...
          case "sketch":
            ...
            for (let i = 0; i < 3; i++) {
              ...
              ctx.lineTo(
                localPos.x + Math.cos(angle) * r,
                localPos.y + Math.sin(angle) * r,
              );
              ctx.stroke();
            }
            break;
  ...
  const handleMouseMove = (e: Konva.KonvaEventObject<MouseEvent>) => {
    if (!isDrawing || (tool !== "eraser" && tool !== "brush") || !selectedId)
      return;
      ...
      switch (brushType) {
        ...
        case "stipple":
          const distance = Math.sqrt(
            Math.pow(localPos.x - localLastPos.x, 2) +
              Math.pow(localPos.y - localLastPos.y, 2),
          );
    ...
    <div className="flex flex-col h-dvh bg-primary overflow-hidden">
                ...
                {images.map((img) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/30 transition-all duration-300 flex items-center justify-center">
                      <span className="opacity-0 group-hover:opacity-100 text-[10px] uppercase tracking-[0.2em] text-gold transition-all duration-300">
                        Select
                      </span>
                    </div>
                  </div>
              ...
              {[
                ...
                >
                  <div className="w-14 h-14 mb-3 flex items-center justify-center">
                    {shape === "circle" && (
                      <div className="w-12 h-12 rounded-full bg-text-muted group-hover:bg-gold transition-colors duration-300" />
                    )}
                    {shape === "square" && (
                      <div className="w-12 h-12 bg-text-muted group-hover:bg-gold transition-colors duration-300" />
                    )}
                    {shape === "rectangle" && (
                      <div className="w-14 h-8 bg-text-muted group-hover:bg-gold transition-colors duration-300" />
                    )}
                    {shape === "triangle" && (
                      <div className="w-0 h-0 border-l-[24px] border-l-transparent border-r-[24px] border-r-transparent border-b-[42px] border-b-text-muted group-hover:border-b-gold transition-colors duration-300" />
                    )}
                    {shape === "star" && (
                      <svg
                        className="w-12 h-12 fill-text-muted group-hover:fill-gold transition-colors duration-300"
                        viewBox="0 0 24 24"
                      >
                  ...
                  <span className="text-[10px] uppercase tracking-[0.18em] text-text-muted group-hover:text-gold transition-colors duration-300">
                    {shape}
                  </span>
                </button>
              ))}
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
        ...
        <div className="w-full lg:w-64 flex-shrink-0 max-h-[45vh] lg:max-h-none bg-secondary border-b lg:border-b-0 lg:border-r border-border flex flex-col overflow-y-auto">
          ...
          <div className="flex flex-col gap-px bg-border flex-1 overflow-y-auto">
            ...
            <div className="bg-secondary p-4 space-y-2">
              <span className="text-[10px] uppercase tracking-[0.22em] text-gold block mb-3">
                Import
              </span>
              <button
            ...
            <div className="bg-secondary p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={zoomIn}
                  className="flex-1 flex items-center justify-center py-2.5 bg-surface border border-border text-text-muted hover:border-border-gold hover:text-gold transition-all duration-300"
                >
                  <ZoomIn size={16} />
                </button>
                <button
                    ...
                    <>
                      <div>
                        ...
                        <p className="text-[10px] text-text-dim mt-2 leading-relaxed">
                          {brushType === "normal" &&
                            "Smooth solid brush for clean lines"}
                          {brushType === "stipple" &&
                            "Dotwork/stippling for shading"}
                          {brushType === "sketch" &&
                            "Rough sketchy lines for outlining"}
                        </p>
                      </div>
            ...
            {selectedId && (
              <div className="bg-secondary p-4 space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={moveLayerUp}
                    className="flex-1 flex items-center justify-center gap-1 py-2.5 bg-surface border border-border text-text-muted text-xs hover:border-border-gold hover:text-gold transition-all duration-300"
                  >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-surface border border-border text-text-muted text-xs hover:border-border-gold hover:text-gold transition-all duration-300"
                  >
            ...
            <div className="bg-secondary p-4 space-y-2">
              ...
              <div className="space-y-1">
                {layers.map((layer, index) => (
                  ...
                  >
                    <span className="text-[10px] uppercase tracking-[0.15em] truncate flex-1">
                      {layer.name}
                    </span>
                    <span className="text-[10px] text-text-dim ml-2 flex-shrink-0">
                      #{layers.length - index}
                    </span>
                  </div>
                ))}
        ...
        <div
          ...
          className="flex-1 flex items-center justify-center bg-primary relative overflow-auto p-4 min-h-0"
        >
          {/* Processing overlays */}
              ...
              <div className="bg-secondary border border-border p-10 w-[90vw] max-w-[400px] relative">
                ...
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-text-muted leading-relaxed">
                    {isRemovingBg &&
                      "Removing the background, please wait a moment..."}
                    {isAddingImage &&
                      "Adding image to canvas, please wait a moment..."}
                    {isCanvaSaving &&
                      "Saving design so you can edit later, please wait a moment..."}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div
            style={{ width: 800 * displayScale, height: 600 * displayScale }}
          >
            <Stage
              width={800}
              height={600}
