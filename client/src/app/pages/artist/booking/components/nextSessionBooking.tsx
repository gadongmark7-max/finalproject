...
export function BookNextSession({
  ...
  const { data: bussinessInfoData } = useQuery({
    ...
    queryFn: async (): Promise<bussinessInfoInterface> => {
      const response = await axiosInstance.get(
        `/account/bussinessInfo/${booking?.bussiness?._id}`,
      );
  ...
  const { data: artistInfoData } = useQuery({
    ...
    queryFn: async (): Promise<artistInfoInterface> => {
      const response = await axiosInstance.get(
        `/account/artistInfo/${booking?.artist?._id}`,
      );
      return response.data;
    },
    enabled: booking.bussiness == null,
    ...
    if (data?.data && date)
      setArtistBookings(
        data.data.filter(
          (e: bookingInterface) =>
            e.status == "active" &&
            e.date == date.toLocaleDateString("en-US").toString(),
        ),
      );
  }, [data, date]);

  const bookMutation = useMutation({
    mutationFn: (data: { newTime: string[]; newDate: string; id: string }) =>
      axiosInstance.post("/booking/bookNextSession", data),
    onSuccess: (response) => {
  ...
  const bookHandler = () => {
    if (!date || !selectStartTime || !user || selectedtime.length == 0)
      return errorAlert("empty date or time");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          hoverText={"Schedule Next Session"}
      ...
      <DialogContent className="sm:max-w-[725px]">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {times.map((item, index) => (
                    <Button
                      key={item}
                      variant={
                        selectedtime.includes(item) ? "default" : "outline"
                      }
        ...
        <DialogFooter>
          <Button className="w-full" onClick={bookHandler}>
            {" "}
            Confirm Booking{" "}
          </Button>
        </DialogFooter>
