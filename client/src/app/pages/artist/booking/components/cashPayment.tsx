...
export function CashPayment({ booking }: { booking: bookingInterface }) {
  ...
  const schema = useMemo(
    () => makeCashPaymentSchema(bookingBalance),
    [bookingBalance],
  );
  ...
  // Numeric views for the read-only receipt render.
  ...
  const addMoney = (value: number) => {
    ...
    setValue("customerPayment", String(current + value), {
      shouldValidate: true,
      shouldTouch: true,
    });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[800px]">
        ...
        <div className="space-y-6">
          {showReceipt && (
            <div
              className="bg-[#fafafa] w-4/6 m-auto p-8 rounded-sm border border-gray-300 font-mono text-sm shadow-sm relative"
              id="receipt"
            >
              {/* Receipt Header */}
            ...
            <>
              <div className="flex items-center justify-between  p-4 border-2 border-secondary ">
                <div className="flex gap-4">
                  ...
                  <div className="">
                    <p className="text-sm  text-gold">Client</p>
