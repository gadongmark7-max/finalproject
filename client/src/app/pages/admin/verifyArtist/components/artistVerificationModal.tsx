...
export function ArtistVerificationModal({
  artistVerification,
  setArtistVerification,
}: {
  artistVerification: artistVerificationInterface;
  setArtistVerification: (data: artistVerificationInterface[]) => void;
}) {
  ...
  const mutation = useMutation({
    mutationFn: (data: {
      ...
      status: string;
      reason: string;
    }) => axiosInstance.post("/account/artistVerification/admin", data),
    onSuccess: (response) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>artist request</DialogDescription>
        </DialogHeader>
