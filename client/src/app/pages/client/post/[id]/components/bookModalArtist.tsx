...
export function ArtistBookModal({
  ...
  const bookMutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      ...
      if (post.downPercentage != 0 && variables.paymentMethod !== "counter") {
        setIsLoading(true);
        ...
        Swal.fire({
          ...
          text:
            variables.paymentMethod === "counter"
              ? "Please pay the down payment over the counter when you arrive. Wait for artist approval."
              : "Please Wait for Artist Approval",
        }).then(() => {
  ...
  const bookHandler = (paymentMethod: PaymentMethod) => {
    setOpen(false);
    bookMutation.mutate({
