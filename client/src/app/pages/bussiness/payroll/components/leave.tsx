...
export function LeaveApprovalModal({ refetch }: { refetch: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: (data: { id: string; status: string; isPaid: boolean }) =>
      axiosInstance.put("/account/attendance/leave", data),
    onSuccess: (response) => {
                  ...
                  <TableRow key={index} className={` transition-colors `}>
                    ...
                    <TableCell className="text-right flex justify-end gap-2">
                      <Button
                        ...
                        onClick={() => handlerAction(record._id, "rejected")}
                      >
                        {" "}
                        Reject{" "}
                      </Button>
                      <Button
                        ...
                        onClick={() => handlerAction(record._id, "recorded")}
                      >
                        {" "}
                        Approve{" "}
                      </Button>
