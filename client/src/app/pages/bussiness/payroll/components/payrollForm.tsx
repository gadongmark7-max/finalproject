...
const getArtistCommision = (
  ...
  booking.forEach((item) => {
    if (item.artist._id == artistId)
      commision += (commisionPercentage / 100) * item.originalPrice;
  });
  return commision;
};
...
export default function PayrollForm({
  ...
  clear: () => void;
  attendance: attendanceInterface[];
  employeeData: payrollEmployeeDataInterface[];
  salaryType: string;
  month: string;
  startDate: string;
  endDate: string;
  ...
  const [payrollData, setPayrollData] = useState(
    employeeData.map((employee) => {
      ...
      const otHrs = getTotalOt(
        employeeAttendance.filter((item) => item.otStatus == "recorded"),
      );
      ...
      switch (salaryType) {
        case "hr":
          basicPay =
            workHrs > hrsNeeded
              ? hrsNeeded * employee.salary
              : workHrs * employee.salary;
        ...
        case "day":
          ...
          if (otHrs != 0)
            otPay = (employee.salary / employee.dailyShipDuration) * otHrs;
          break;

        case "month":
          basicPay =
            employeeAttendance.length == shipDays
              ? employee.salary
              : (employee.salary / shipDays) * employeeAttendance.length;
          if (otHrs != 0) {
  ...
  const mutation = useMutation({
    mutationFn: (data: {
      payroll: payRollInterfaceInput;
      isApprove: boolean;
    }) => axiosInstance.post("/account/payroll", data),
    onSuccess: (response) => {
  ...
  return (
    <div className="w-full bg-white p-10 text-black">
      ...
      <div className="text-center border-b-2 border-black pb-4 mb-6">
        <h1 className="text-3xl font-bold tracking-wide">PAYROLL REGISTER</h1>
        ...
        <div className="flex justify-between">
          ...
          <p className="text-lg font-bold">Payroll Type: per {salaryType}</p>
        </div>
      </div>
          ...
          <tbody>
            {payrollData.map((employee, index) => (
              <tr key={employee.employeeId}>
                ...
                <td className="border border-black p-2">{employee.role}</td>
