...
const MapLocation: React.FC<ClickableMapProps> = ({
  ...
  const defaultPosition: L.LatLngExpression = [
    14.315885007395133, 120.94680688824083,
  ];

  const updateMutation = useMutation({
    mutationFn: (location: { lat: number; long: number }) =>
      axiosInstance.put(`/account/location/${bussinessInfo.bussiness._id}`, {
        location,
        artistId: bussinessInfo._id,
      }),
    onSuccess: (response) => {
        ...
        {!open && (
          <div className="w-full h-full rounded" onClick={() => setOpen(true)}>
            <MapContainer
              ...
              center={
                location ? [location.lat!, location.long!] : defaultPosition
              }
              zoom={13}
            ...
            >
              ...
              {location && (
                <Marker
                  ...
                  icon={mapIcon(bussinessInfo.bussiness.profile)}
                ></Marker>
              )}
            </MapContainer>
          </div>
      ...
      <DialogContent className="sm:max-w-[825px] h-[600px]">
        <div className=" gap-6 w-full h-[500px] mt-10">
          ...
          >
            ...
            {mapLocation && (
              <Marker
                position={[mapLocation.lat, mapLocation.long]}
                icon={mapIcon(bussinessInfo.bussiness.profile)}
              ></Marker>
            )}
          </MapContainer>
