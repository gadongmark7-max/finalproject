...
export function ViewPayroll({
  ...
  const approveMutation = useMutation({
    mutationFn: (data: { id: string; approveBy: string }) =>
      axiosInstance.put("/account/payroll/approve", data),
    onSuccess: (response) => {
  ...
  const rejectMutation = useMutation({
    mutationFn: () =>
      axiosInstance.put(`/account/payroll/reject/${payroll._id}`),
    onSuccess: (response) => {
      ...
      <DialogContent className="sm:max-w-[1025px]  bg-white">
          ...
          <div className="flex justify-between mt-5 text-secondary">
            {payroll.payroll[0].payType === "month" ? (
              <p className="text-lg font-bold">
                Month: {payroll.payroll[0].month}
              </p>
            ) : (
              <p className="text-lg font-bold">
                Period Covered: {payroll.payroll[0].periodFrom} to{" "}
        ...
        <div className="mt-10 grid grid-cols-3 gap-10 text-sm">
          <div className="text-center">
            <p className="font-medium">{payroll?.preparedBy}</p>
          ...
          <div className="text-center">
            <p className="font-medium">{payroll?.approveBy || "."}</p>
        ...
        <DialogFooter>
          {payroll.status == "pending" && (
            <>
              <Button
                className="bg-red-500 hover:bg-red-600 shadow"
